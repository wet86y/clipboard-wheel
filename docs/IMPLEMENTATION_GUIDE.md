...
管理员模式下，普通权限的 Windows 资源管理器不能进入 WPF 的 OLE 拖放链路。点击应用模式的空槽拖入框后，主进程使用 Explorer 的普通权限令牌启动同一 EXE 的一次性拖放辅助窗口；辅助窗口接收 `.lnk` 后，把路径原子写入当前用户 `%LOCALAPPDATA%` 下由随机 128 位令牌命名的一次性交接文件，管理员主进程只读取对应文件并立即删除。主进程必须再次校验当前模式、槽位为空、文件存在且扩展名为 `.lnk`。辅助模式在单实例互斥锁之前分流，不启动托盘、鼠标钩子或剪贴板服务。
