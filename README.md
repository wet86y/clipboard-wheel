...
| 突破轮盘 | 圆形模式下突破外缘可唤出外圈动作槽，支持快捷键、`.lnk` 快捷方式，以及直接拖入程序、文件和文件夹；未配置槽位不参与选中并回落到内圈选择 |
| 剪贴板历史 | 启动后台读取 Win+V 历史补底，运行期自己监听并置顶，维护最近 8 条 |
| 多格式 | 文本 / HTML / RTF / CSV / TSV，Excel/WPS 表格区域粘贴保留格式；可选清洗表格的纯文本载荷 |
...
2.1.1 扩展了表格内容的纯文本粘贴控制，并让突破轮盘可以直接接收程序、文件和文件夹。

- **可选的表格文本清洗**：基础设置新增默认关闭的开关。开启后，仅在已识别为表格的内容以纯文本粘贴时移除横向空白和中英文单双引号，保留原有换行；HTML、RTF、CSV 等表格格式不变。
- **表格结构保持兼容**：Excel/WPS 选中表格区域时仍按原格式粘贴；进入单元格编辑状态或粘贴到文本框时，才使用清洗后的纯文本。
- **直接拖入突破轮盘**：应用槽现在可接收单个程序、普通文件或文件夹，并自动创建受管快捷方式；已有 `.lnk` 仍沿用原路径。
- **安全的快捷方式生命周期**：候选快捷方式在设置保存成功后才生效，取消、保存失败、重新绑定和清除槽位都会按受管目录边界清理，绝不删除源文件或外部快捷方式。
- **纯净的正式资产**：Release 仍为单 EXE，不包含诊断日志、PDB 或故障注入。
